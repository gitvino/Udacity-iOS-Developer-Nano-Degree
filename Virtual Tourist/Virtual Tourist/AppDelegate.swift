...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        DatabaseManager.sharedInstance.dataController = dataController
        
        
        let fm = FileManager.default
        if let documentsPathURL = fm.urls(for: .documentDirectory, in: .userDomainMask).first {
            
            let folderPath =  documentsPathURL.appendingPathComponent("DownloadedImages")
            if !fm.fileExists(atPath: folderPath.absoluteString) {
                do {
